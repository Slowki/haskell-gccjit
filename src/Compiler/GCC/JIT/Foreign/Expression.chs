...
contextNewFunction c l k rt n ps isvar = withArray (map extractPointer ps) $
    \ar -> useAsCString n $
        \cs -> {#call unsafe gcc_jit_context_new_function#} c (fromMaybeLocation l) (enumToCInt k) rt cs (fromIntegral $ length ps) (castPtr ar :: Ptr JITParam) (if isvar then 1 else 0)
    where
